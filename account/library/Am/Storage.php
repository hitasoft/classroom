...
class Am_Storage_File extends Am_Storage_Item
{
    ...
    public function getUrl($exptime = null)
    {
        return $this->storage->getUrl($this, $exptime);
    }
...
class Am_Storage_Disk extends Am_Storage
{
    ...
    {
        if (!file_exists($this->root))
            return ___("Error : upload folder [%s] does not exists", $this->root);
        else
            return ___("Upload files using FTP client to [%s]", $this->root);
    }
    ...
    public function checkPath($path)
    {
        ...
        {
            $path = Am_Controller::escape($path);
            $root = Am_Controller::escape($this->root);
        ...
        foreach ($usage as $msg) {
            if ($msg['link']) {
                $str .= sprintf('<li><a href="%s" target="_blank">%s</a></li>',
                        Am_Controller::escape(REL_ROOT_URL . $msg['link']), Am_Controller::escape($msg['title']));
            } else {
                $str .= sprintf('<li>%s</li>',
                    Am_Controller::escape($msg['title']));
            }
        }
...
$("#delete-back-link").die().live('click', function(){
...
class Am_Storage_Grid
{
    ...
    function __construct(Am_Storage $storage, Am_Request $request, Zend_Controller_Response_Http $response, array $plugins)
    {
        $this->storage = $storage;
    ...
    public function getUrl($path)
    {
        ...
        $get['path'] = $path;
        return REL_ROOT_URL . '/admin-upload/grid?' . http_build_query($get, '', '&');        
    }
