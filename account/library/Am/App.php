...
class Am_Theme_Default extends Am_Theme
{
    ...
    public function initSetupForm(Am_Form_Setup_Theme $form)
    {
        ...
        $form->addTextarea('header', array('rows' => 7, 'style' => 'width:95%'))
                ->setLabel(array(___('Header'), ___('It will be included to header. It can contain html markup')))->default = '';
    }
...
class Am_Cron
{
    ...
    static function checkCron()
    {
        ...
        if ($needRun & self::WEEKLY) {
            Am_Di::getInstance()->hook->call(Am_Event::WEEKLY, array('datetime' => sqlDate('now')));
            $out .= "weekly.";
        }
        if ($needRun & self::MONTHLY) {
            Am_Di::getInstance()->hook->call(Am_Event::MONTHLY, array('datetime' => sqlDate('now')));
            $out .= "monthly.";
        }
        if ($needRun & self::YEARLY) {
            Am_Di::getInstance()->hook->call(Am_Event::YEARLY, array('datetime' => sqlDate('now')));
            $out .= "yearly.";
        }
        if (!empty($_GET['log']))
...
//            Zend_Translate::setCache($cache);
        ...
        foreach (array($locale, $lang) as $l)
 ...
 if (!defined('ROOT_DIR'))
    ...
    public function resize($handler, $width, $height, $resize_type = self::RESIZE_CROP)
    {
        ...
        switch ($resize_type) {
            ...
            case self::RESIZE_CROP:
                ...
                break;
            default:
                throw new Am_Exception_InputError(sprintf('Unknown resize type [%s] in %s->%s',
