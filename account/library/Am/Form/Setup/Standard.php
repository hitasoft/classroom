...
class Am_Form_Setup_Global extends Am_Form_Setup
{
    ...
    function initElements()
    {
        ...
        */
        ...
        $loginLen->addStatic('')->setContent('<span>&mdash;</span>');
        ...
        $passLen->addStatic('')->setContent('<span>&mdash;</span>');
        ...
        $this->addSelect('404_page')
            ->setLabel(array(___('Page Not Found (404)'), '<strong>' . ___('this page will be public and do not require any login/password') . "</strong>\n" .
                ___('you can create new pages %shere%s', '<a href="' . REL_ROOT_URL . '/default/admin-content/p/pages/index' . '">', '</a>')))
...
class Am_Form_Setup_Email extends Am_Form_Setup
{
    ...
    function initElements()
    {
        ...
        $fs->addElement('integer', 'smtp_port', null,  array('help-id' => '#SMTP_Mail_Settings'))
...
$em <span class='admin-help'><a href='http://v4.amember.com/docs/Setup/Email#Test_Email_Settings' target='_blank'><sup>?</sup></a></span><input type='text' name='email' size=30 />
...
CUT
        ...
        ->setLabel(array(___('Send Cancel Notifications to Admin'), ___('send email to admin when recurring subscription<br />cancelled by member.')));
        ...
        ->setLabel(array(___('Change Password Notification'), ___('send email to user after password change')));
        ...
        if($this->haveCronRebillPlugins())
        {
            ...
            ->setLabel(___('Send Credit Card Rebill Stats to Admin', 'Credit Card Rebill Stats willbe sent to Admin daily<br/>
...
CUT
        ...
        $fs->addTextarea('unsubscribe_html', array('style'=>'width:95%', 'rows'=>6),  array('help-id' => '#Miscellaneous_Email_Settings'))
        ...
        $fs->addTextarea('unsubscribe_txt', array('style'=>'width:95%', 'rows'=>6),  array('help-id' => '#Miscellaneous_Email_Settings'))
...
class Am_Form_Setup_Pdf extends Am_Form_Setup
{
    function __construct()
    {
        ...
        $info = ___('You can find info regarding pdf invoice customization %shere%s', '<a target="_blank" href="http://www.amember.com/docs/How_to_customize_PDF_invoice_output">', '</a>');
        $this->addProlog(<<<CUT
...
CUT
    ...
    function initElements()
    {
        ...
        $this->addElement('text', 'invoice_filename', array('size'=>30))
        ->setLabel(___('Filename for Invoice') . "\n" . ___('%public_id% will be replaced with real public id of invoice, %receipt_id% will be replaced with payment receipt'));
...
class Am_Form_Setup_VideoPlayer extends Am_Form_Setup
{
    ...
    public function setupElements(Am_Form $form, $prefix = null) {
        $form->addElement('text', $prefix . 'width')
        ->setLabel(___('Default Width'));

        $form->addElement('text', $prefix . 'height')
        ->setLabel(___('Default Height'));
        ...
        $form->addElement('text', $prefix . 'bufferLength')
...
CUT
        ...
        $gr->addElement('integer', 'clear_access_log_days', array('class'=>'clear_access_log_days'));
...
CUT
        ...
        $gr->addElement('integer', 'clear_inc_payments_days', array('class'=>'clear_inc_payments_days'));
...
CUT
        ...
        $this->addText('multi_title', array('style'=>'width:95%'), array('help-id' => '#Configuring_Advanced_Settings'))
...
CUT;
        ...
        $fs->addTextarea('manually_approve_note', array('rows' => 8, 'style'=>'width:95%'))
...
CUT
        ...
        $gr->addStatic()->setContent(' ' . ___('minutes deny access for user and do the following'));
        ...
        $this->addScript()
            ->setScript(<<<CUT
$('#skip-index-page').change(function(){
    $('#index-page').closest('.row').toggle(!this.checked)
}).change();
CUT
            );
        ...
        $this->addSelect('login_recaptcha_theme', '', array(
         ...
         $this->addSelect('video_non_member')
            ->setLabel(array(___('Video for Non Member'), ___('this video will be shown instead of actual video in case of non-member try to access protected video content') . "\n" . '<strong>' . ___('this video will be public and do not require any login/password') . "</strong>\n" .
                ___('you can add new video %shere%s', '<a href="' . REL_ROOT_URL . '/default/admin-content/p/video/index' . '">', '</a>')))
         ...
         $this->addSelect('video_not_proper_level')
            ->setLabel(array(___('Video for Member without Proper Membership Level'), ___('this video will be shown instead of actual video in case of member without proper access try to access protected video content') . '<strong>' . ___('this video will be public and do not require any login/password') . "</strong>\n" .
                ___('you can add new video %shere%s', '<a href="' . REL_ROOT_URL . '/default/admin-content/p/video/index' . '">', '</a>')))
            ->loadOptions(array(''=>___('Show Error Message')) +
                Am_Di::getInstance()->db->selectCol("SELECT video_id AS ?, title FROM ?_video", DBSIMPLE_ARRAY_KEY));
    }
...
class Am_Form_Setup_Recaptcha extends Am_Form_Setup
{
    ...
    function initElements()
    {
        $this->addText("recaptcha-public-key", array('size'=>50), array('help-id' => 'Setup/ReCaptcha'))->setLabel("<a href='http://www.google.com/recaptcha' target='_blank'>ReCaptcha</a> Public Key")
        ->addRule('required', ___('This field is required'));
        $this->addText("recaptcha-private-key", array('size'=>50), array('help-id' => 'Setup/ReCaptcha'))->setLabel("<a href='http://www.google.com/recaptcha' target='_blank'>ReCaptcha</a> Private Key")
        ->addRule('required', ___('This field is required'));
