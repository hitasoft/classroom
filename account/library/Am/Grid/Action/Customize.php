...
class Am_Grid_Action_Customize extends Am_Grid_Action_Abstract
{
    ...
    public function run()
    {
        ...
        $form->addSortableMagicSelect('fields')
            ...
            ->setLabel(___('Fields to Disply in Grid'))
            ->setJsOptions(<<<CUT
{
    allowSelectAll:true,
    sortable: true
...
CUT
        ...
        if ($form->isSubmitted()) {
            ...
            echo sprintf('<div class="info">%s</div>',
                ___('You can change Number of %sRecords per Page%s in section %sSetup/Configuration%s',
                    '<strong>', '</strong>',
                    '<a href="' . REL_ROOT_URL . '/admin-setup" target="_top">','</a>'));
            echo $form;
        }
    }
    ...
    public function setGrid(Am_Grid_Editable $grid)
    {
        ...
        $grid->addCallback(Am_Grid_Editable::CB_INIT_GRID, array($this, 'setupFields'));
        parent::setGrid($grid);
    }

    public function renderLink(& $out)
    {
        $out .= sprintf('<div style="float:right">&nbsp;&nbsp;<a href="%s">' . ___('Customize') . '</a></div>',
                $this->getUrl());
    }
