...
CUT
        ...
        if (in_array(Am_Paysystem_CreditCard::CC_CODE, $options))
        {
            $code = $this->addPassword('cc_code', array('autocomplete'=>'off', 'size'=>4, 'maxlength'=>4))
                    ->setLabel(array(___('Credit Card Code'), sprintf(___('The "Card Code" is a three- or four-digit security code%sthat is printed on the back of credit cards in the card\'s%ssignature panel (or on the front for American Express cards).'),'<br>','<br>')));
        ...
        if (in_array(Am_Paysystem_CreditCard::CC_ADDRESS, $options))
        {
            ...
            {
                $street2 = $fieldSet->addText('cc_street2')->setLabel(___('Street Address (Second Line)'))
                               ->addRule('required', ___('Please enter Street Address'));
            }
