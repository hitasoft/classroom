...
class Am_Paysystem_PaymentExpress extends Am_Paysystem_CreditCard
{
    ...
    protected function _initSetupForm(Am_Form_Setup $form)
    {
        $form->addText('PxPayUserId')
            ->setLabel(array('Your PxPayUserId',
                ''))
            ->addRule('required');
        
        $form->addText('PxPayKey', array('size' => 65))
            ->setLabel(array('Your PxPayKey',
                ''))
            ->addRule('required');
        
        $form->addText('PostUsername')
            ->setLabel(array('Your PostUsername',
                ''))
            ->addRule('required');
        
        $form->addPassword('PostPassword')
            ->setLabel(array('Your PostPassword',
                ''))
            ->addRule('required');
        
        $form->addAdvCheckbox('debugMode')
            ->setLabel(array("Debug Mode Enable" ,
...
class Am_Paysystem_Transaction_CreditCard_PaymentExpress_Refund extends Am_Paysystem_Transaction_CreditCard_PaymentExpress_Rebill
{
    ...
    public function __construct(Am_Paysystem_Abstract $plugin, Invoice $invoice, $dpsBillingId, $amount)
    {
        $this->amount = $amount;
        parent::__construct($plugin, $invoice, false, $dpsBillingId);
    }
