...
class Am_Paysystem_Payforit extends Am_Paysystem_CreditCard
{
    ...
    public function _initSetupForm(Am_Form_Setup $form)
    {
        $form->addText("company")
            ->setLabel(array("Your company name in payforit platform",
                ""))
            ->addRule('required');

        $form->addText("password", array('size' => 40))
            ->setLabel(array("Your password in payforit platform",
                ""))
            ->addRule('required');

        $form->addSelect("window")
            ->setLabel(array("Type of Payforit Window ",
...
class Am_Paysystem_Transaction_Payforit extends Am_Paysystem_Transaction_Incoming_Thanks
{
    ...
    {
         return $this->request->getInt('transactionId') . '-' . Am_Di::getInstance()->app->generateRandomString(4);
    }
