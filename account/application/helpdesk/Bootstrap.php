...
class Bootstrap_Helpdesk extends Am_Module
{
    ...
    function renderNotification()
    {
        if ($user_id = $this->getDi()->auth->getUserId()) {
            ...
            if ($cnt)
                return '<div class="am-info">' . ___('You have %s%d tickets%s that require your attention',
    ...
    function onAdminWarnings(Am_Event $event)
    {
        ...
        if ($cnt)
            $event->addReturn(___('You have %s%d tickets%s that require your attention',
                    sprintf('<a href="%s">', REL_ROOT_URL . '/helpdesk/admin/?_admin_filter_s[]=new&_admin_filter_s[]=awaiting_admin_response'), $cnt, '</a>'));
    }
