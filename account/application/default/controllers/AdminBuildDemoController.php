...
//http://en.wikipedia.org/wiki/List_of_cities,_towns,_and_villages_in_the_United_States
     ...
     */
    public function createInvoices($user, $payplugin, $product_ids, $invCnt, $invVar, $prdCnt, $prdVar, $start, $end) {
        ...
        for($j=1; $j<=$invoiceLimit; $j++) {
            ...
            for ($k=1; $k<=$productLimit; $k++) {
                try {
                    $invoice->add(Am_Di::getInstance()->productTable->load(array_rand($product_ids, 1)), 1);
            ...
            if ($invoice->isZero()) {
                ...
                $tr->setAmount($invoice->first_total)
                    ->setInvoice($invoice)
                    ->setTime(new DateTime('@' . $tm))
                    ->setReceiptId('demo-'. microtime(true))
                    ->process();
                ...
                while ((float)$invoice->second_total 
                    ...
                    $tr->setAmount($invoice->second_total)
                        ...
                        ->setReceiptId('demo-rebill-' . $i++ . '-' . microtime(true))
                        ->process();
