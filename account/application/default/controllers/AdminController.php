...
class AdminController extends Am_Controller
{
    ...
    {
        $form = new Am_Form_Admin();
        $form->addMagicSelect('interval', null, array('options' => $this->getDi()->interval->getOptions()))
            ->setLabel(___('Period'))
    ...
    function getWarnings()
    {
        ...
        if (!$this->getDi()->config->get('maintenance'))
        {
            ...
            if ($diff > 24 * 3600)
                ...
                You can find info how to set up cron job for your installation <a href="http://www.amember.com/docs/Cron" target="_blank">here</a>.', $tt);
        }
        ////
        if (!$this->getDi()->productTable->count())
            $warn[] = ___('You have not added any products, your signup forms will not work until you <a href="admin-products">add at least one product</a>');
