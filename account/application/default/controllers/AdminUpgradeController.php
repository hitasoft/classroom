    ...
    public function stepLoadUpgradesList()
    {
        ...
        if (!$this->getSession()->upgrades)
            throw new Am_Exception_InputError(
                ...
                ". <a href='".REL_ROOT_URL."/admin-upgrade?reset=1&beta=1'>" . 
...
//                return false;
    ...
    function loadUpgradesList($requireAuth = false)
    {
        ...
        $req->addPostParameter('timezone-check',  
                $this->getDi()->db->selectCell('SELECT @@global.time_zone') . '---' .
                $this->getDi()->db->selectCell('SELECT @@system_time_zone') . '---' .
                date_default_timezone_get() . '---' .
                time() . '---' .
                gmdate('Y-m-d H:i:s') . '---' . 
		$this->getDi()->db->selectCell('SELECT NOW()') . '---' .
		$this->getDi()->db->selectCell('SELECT UTC_TIMESTAMP()').'---' 
        );
