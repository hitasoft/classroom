...
class SettingsController extends Am_Controller
{
    /** @var int */
    protected $user_id;
    /** @var User */
    ...
    function preDispatch()
    {
        if ($this->getRequest()->getActionName() != 'confirm-email') {
            $this->getDi()->auth->requireLogin(ROOT_URL . '/settings');
    ...
    function indexAction()
    {
        ...
        if (!$record)
            throw new Am_Exception_Configuration("No settings form configured");
        ...
        if ($this->form->isSubmitted() && $this->form->validate())
        {
            ...
            $msg = $ve ? ___('Verification email has been sent to your address.
                    ...
                    ___('Your settings have been updated successfully');
        ...
        $this->view->display('member/settings.phtml');
    }
    ...
    protected function handleEmail(SavedForm $form, & $vars) {
        ...
        foreach ($bricks as $brick) {
            if ($brick->getClass() == 'email' 
                ...
                $tpl->setUser($user);
                $tpl->setCode($code);
                $tpl->setUrl($this->getDi()->config->get('root_surl') . '/settings/confirm-email?em=' . $user->pk() . ':' . $code);
                $tpl->send($user);
