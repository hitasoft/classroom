...
class AjaxController extends Am_Controller
{
    ...
    function ajaxCheckUniqLogin($vars){
		...
		$folder=file_exists($_SERVER['DOCUMENT_ROOT'].'/../'.$login);
        if (!$this->getDi()->userTable->checkUniqLogin($login, $user_id) || $folder)
            $msg = ___('Username %s is already taken.', Am_Controller::escape($login));
    ...
    function ajaxCheckUniqEmail($vars){
        ...
        if (!$this->getDi()->userTable->checkUniqEmail($email, $user_id))
            $msg = ___('An account with the same email already exists. ').
                    ___('Please %slogin%s to your existing account.','<a href="'.Am_Controller::escape(REL_ROOT_URL . '/member').'">','</a>');
