...
class Am_Form_Admin_Product extends Am_Form_Admin {
    ...
    function init() {
        ...
        $gr->addStatic()->setContent('<div><a href="'.Am_Controller::escape(REL_ROOT_URL).'/admin-product-categories">'.___('Edit Categories').'</a></div>');
...
class AdminProductsController extends Am_Controller_Grid
{
    ...
    public function createGrid()
    {
        ...
        $grid->addGridField(new Am_Grid_Field('product_id', '#', true, '', null, '1%'));
        ...
        $grid->addGridField(new Am_Grid_Field('terms', ___('Billing Terms'), false))->setRenderFunction(array($this, 'renderTerms'));
        if ($this->getDi()->plugins_tax->getEnabled()) {
            $grid->addGridField(new Am_Grid_Field('tax_group', ___('Tax')));
