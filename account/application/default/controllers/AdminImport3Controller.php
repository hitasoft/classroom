...
class Am_Import_Product3 extends Am_Import_Abstract
{
    ...
    public function doWork(&$context)
    {
		...
		$data = preg_replace_callback(
            ...
            if(empty($p->currency)) $p->currency = Am_Currency::getDefault();
            ...
            if (in_array('subusers', $this->getDi()->modules->getEnabled()))
            {
                foreach ($r as $k => $v)
                {
                    ...
                    {
                        $p->subusers_count = intval($v);
                        $p->data()->set('subusers_groups', $match[1]);
                        break;
                    }
                }
                if (@$this->options['keep_product_id'] && !empty($r['additional_subscriptions']))
                    $p->subusers_product_id = $r['additional_subscriptions'][0]; // only first product
            }
                ...
                if(!empty($r['data']['subusers_groups_id']))
                {
                    ...
                    if(!empty($subusers_groups_id))
                    {
                        ...
                        foreach ($this->getDi()->db->selectCol("
                                SELECT `id` FROM ?_data WHERE `table`='product' AND `value` IN (?a)
                            ", $subusers_groups_id) as $masterProductId)
                        {
                            $prId = $this->getDi()->productTable->load($masterProductId)->subusers_product_id;
                            $sql[] = "(" . $prId . "," . $u->pk() . ")";
                        }
...
EOL
    ...
    function cleanUpData($value){
       ...
       $this->redirectHtml(REL_ROOT_URL . '/admin-import3', 'Records removed');
    }
        ...
        $form->addStatic()
            ...
            ->setContent(
                sprintf(___('Use this %slink%s to delete data from aMember v4 database and use clean database for import'), 
                    '<a href="'.REL_ROOT_URL.'/admin-import3/clean">', '</a>'));
        $cb = $form->addGroup();
