...
class Am_Form_Element_PlayerConfig extends HTML_QuickForm2_Element
{
    ...
    {
        return sprintf('<div class="player-config">%s%s <div class="player-config-edit"><a href="javascript:;">%s</div><div class="player-config-delete"><a href="javascript:;">%s</div><div class="player-config-save"><a href="javascript:;">%s</a></div></div>',
...
CUT
        ...
        $form->addText('title', array('style' => 'width:95%'))->setLabel(___('Title'))->addRule('required', 'This field is required');
        $form->addText('desc', array('style' => 'width:95%'))->setLabel(___('Description'));
...
class Am_Grid_Editable_Pages extends Am_Grid_Editable_Content
{
    ...
    function createForm()
    {
        ...
        $form->addText('title', array('style' => 'width:95%'))->setLabel(___('Title'))->addRule('required', 'This field is required');
        $form->addText('desc', array('style' => 'width:95%'))->setLabel(___('Description'));
        $form->addText('path', array('style' => 'width:95%'))
            ->setId('page-path')
...
$('#page-path').bind('keyup', function(){
    ...
    $('#page-permalink').html($('#page-permalink').data('root_url') + '/' + encodeURIComponent($(this).val()))
}).trigger('keyup')
CUT
        );
        ...
        $form->addSelect('tpl')
            ...
            ->loadOptions($this->getPageTemplates());
        $form->addScript()
            ->setScript(<<<CUT
...
CUT
        ...
        $fs = $form->addAdvFieldset('meta')
                ->setLabel(___('Meta Data'));

        $fs->addText('meta_title', array('style' => 'width:95%'))
            ->setLabel(___('Title'));

        $fs->addText('meta_keywords', array('style' => 'width:95%'))
            ->setLabel(___('Keywords'));

        $fs->addText('meta_description', array('style' => 'width:95%'))
...
class Am_Grid_Editable_Links extends Am_Grid_Editable_Content
{
    ...
    {
        $form = new Am_Form_Admin;
        $form->addText('title', array('style' => 'width:95%'))->setLabel(___('Title'))->addRule('required');
        $form->addText('desc', array('style' => 'width:95%'))->setLabel(___('Description'));
        $form->addText('url', array('style' => 'width:95%'))->setLabel(___('URL'))->addRule('required');
        $form->addAdvCheckbox('hide')->setLabel(___("Hide\n" . "do not display this item link in members area"));
...
class Am_Grid_Editable_Integrations extends Am_Grid_Editable_Content
{
    ...
    protected function initGridFields()
    {
        ...
        $this->addGridField('resource', ___('Resource'), false)->setRenderFunction(array($this, 'renderResourceTitle'));
        parent::initGridFields();
...
class Am_Grid_Editable_Folders extends Am_Grid_Editable_Content
{
    ...
    function createForm()
    {
        ...
        $title = $form->addText('title', array('style' => 'width:95%'))->setLabel(___("Title\ndisplayed to customers"));
...
class Am_Grid_Editable_Emails extends Am_Grid_Editable_Content
{
    ...
    public function createForm()
    {
        ...
        $form->addText('recipient_emails', array('style' => 'width:95%', 'id' => 'input-recipient-emails', 'placeholder' => ___('Email Addresses Separated by Comma')))
            ->setLabel(___('Emails'))
            ->addRule('callback2', ___('Please enter valid e-mail addresses'), array($this, 'validateOtherEmails'));

        $form->addText('bcc', array('style' => 'width:95%', 'placeholder' => ___('Email Addresses Separated by Comma')))
            ->setLabel(___('BCC'))
...
CUT
        ...
        $form->addEpilog('<div class="info">' . ___('In case of video do not start play before
                full download and you use <a href="http://en.wikipedia.org/wiki/MPEG-4_Part_14">mp4 format</a>
                ...
                download. Also your video editor can has option to locate metadata at beginning of file
                (something like FastStart or Web Optimized option). You need to relocate metadata for this file and reupload
                it to aMember. You can use such utilites as AtomicParsley or similar
