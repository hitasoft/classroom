...
class Am_Grid_Action_Group_PasswordConfirmedDelete extends Am_Grid_Action_Group_Delete
{
    ...
    public function renderConfirmationForm($btn = null, $page = null, $addHtml = null)
    {
        if (!$this->getSession()->login_ok)
            $addHtml = "<input type='password' name='_admin_pass' size=20/>" . $addHtml;
        else
            $addHtml = null;
...
class Am_Form_Admin_User extends Am_Form_Admin {
    ...
    function init()
    {
        ...
        $login = $fieldSet->addText('login', array('size' => 20))
        ...
        if ($this->record->isLoaded()) {
            ...
            $fieldSet->addStatic('_signup_info', null, array('label' => ___('Signup Info')))->setContent(
                sprintf('<div>%s %s</div>%s', $this->record->remote_addr . ___(' at ') . amDatetime($this->record->added), $banInfoHtml,
                    $countSameIp ? '<div>' . ___('There is %s users with same registration IP Address',
                        '<a href="' . REL_ROOT_URL . '/admin-users?_u_filter=' . $this->record->remote_addr . '">' . $countSameIp . '</a>') . '</div>' : '')
            );
            ...
            $fieldSet->addStatic('_signin_info', null, array('label' => ___('Last Signin Info')))->setContent(
                sprintf("<div>%s</div>", $this->record->last_login ? $this->record->last_ip . ___(' at ') . amDatetime($this->record->last_login) . $banInfoHtml : ___('Never'))
            );
        }


        $comment = $fieldSet->addTextarea('comment', array('style'=>"width:95%", 'id' => 'comment'), array('label' => ___('Comment')));
...
* subscription expiration notices"))
        ...
        $group = $fieldSet->addGroup('', array('id' => 'user_groups'))->setLabel(___('User Groups'));
        ...
        $group->addHtml()->setHtml(sprintf('<div><a href="%s" target="_blank">%s</a></div>',
...
//        $grid->addAction(new Am_Grid_Action_Group_Callback('email', ___("E-Mail"), array($this, 'email')));
    ...
    function renderPayments(User $record)
    {
        if ($record->payments_count)
        {
            ...
            $text = sprintf('<a target="_top" href="%s#payments">%s</a>', $link, $text);
        } else
            $text = ___('Never');
