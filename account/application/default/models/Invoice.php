...
class Invoice extends Am_Record_WithData
{
 ...
 $item->add($qty);
    ...
    function calculate()
    {
        $this->first_period = $this->second_period = null;
...
//                    return $this->_getItemCompatibleError('SECONDPERIOD', $item, $existingItem);
    ...
    function render($indent = "", InvoicePayment $payment = null)
    {
        ...
        foreach ($this->getItems() as $item) {
            $out .= $indent . sprintf("  %-30s %12s%7s %3s%8.2f\n", ___($item->item_title), $item->qty . 'x' . sprintf('%.2f', $item->{$prefix . '_price'}), ' ', $this->currency, $item->{$prefix . '_total'});
        }
        $out .= $indent . str_repeat('-', 70) . "\n";
